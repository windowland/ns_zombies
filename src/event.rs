...
lazy_static::lazy_static! {
  ...
  static ref ZOMBIE:Regex = Regex::new(r#"^@@(?P<to>[a-z0-9_\-]*)@@ was ravaged by a Zombie (?P<level>[a-zA-Z]*) Horde from @@(?P<from>[a-z0-9_\-]*)@@, infecting (?P<affected>[\d,]*) million survivors((\.)|(?P<convert> and converting to a zombie exporter! Oh no!))$"#
  ).unwrap();
  ...
  static ref MOVE:Regex = Regex::new(r#"^@@(?P<nation>[a-z0-9_\-]*)@@ relocated from %%(?P<from>[a-z0-9_\-]*)%% to %%(?P<to>[a-z0-9_\-]*)%%\.$"#).unwrap();
}
impl<'a> ZEvent<'a> {
